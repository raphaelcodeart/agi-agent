...
this first version (WhatsApp Business Cloud API, Instagram Direct via the
Meta Graph API - both require app review/business verification that can't
be completed from inside this codebase). Kept as real classes registered in
the connector registry, per spec section 4/36, so wiring up the real
implementation later is a one-file change with no changes anywhere else in
the module - never invents endpoints, just refuses clearly until the real
integration is implemented.

Facebook Messenger is no longer a stub - see connectors/facebook.py.
"""
