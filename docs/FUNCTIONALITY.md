...
Instagram e Facebook non hanno un numero altrettanto stabile/documentato lato Buffer, quindi per questi due non c'è un controllo proattivo sulla durata — pubblicano sempre e solo come **"post" normale (feed)**, mai come Reel (`prod_client.py`, `metadata.instagram.type`/`metadata.facebook.type` sono hardcoded a `"post"`). Questo non è una limitazione: è la scelta corretta, perché un post normale accetta video molto più lunghi di un Reel (Instagram: post fino a ~60 minuti contro ~90 secondi per un Reel via API; Facebook: post fino a ~240 minuti contro ~90 secondi-2 minuti per un Reel) — quindi il formato "post" è già quello che tollera meglio i video lunghi, nessun fallback verso Reel serve o aiuta. Una versione precedente di questa logica tentava un fallback automatico verso Reel quando "post" falliva per un errore che sembrava di durata: è stata rimossa perché concettualmente all'incontrario (Reel è sempre più restrittivo di un post, mai meno) e rischiosa (es. Facebook rifiuta i Reel senza traccia audio, quindi un video muto avrebbe fallito per un motivo nuovo dopo il retry). Se in futuro serve dare all'amministratore la scelta esplicita di pubblicare come Reel (es. per la spinta algoritmica), va aggiunta come impostazione per-campagna consapevole del limite di durata più stretto — non come fallback automatico da un "post" fallito.
